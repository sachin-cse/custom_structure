            ...
            <div class="col-sm-3">
                <div class="card">
                    <div class="card-body text-center">
                        <a href="<?php echo SITE_ADMIN_PATH.'/index.php?pageType='.$this->_request['pageType'].'&dtls='.$this->_request['dtls'].'&dtaction=add&moduleId='.$this->_request['moduleId'];?>" class="btn btn-info">Add Event</a>
                    </div>
                </div>
            </div>
        </div>
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <tbody class="swap">
                                                ...
                                                foreach($data['events'] as $event) {
                                                    ...
                                                    ?>
                                                    <tr id="<?php echo 'recordsArray_'.$event['id'];?>">
                                                        <td width="40">
                                                            <input type="checkbox" name="selectMulti[]" value="<?php echo $event['id'];?>" class="case" />
                                                        </td>
                                                        
                                                        <td width="40" scope="row"><?php echo $slNo;?></td>
                                                        
                                                        <td width="30" scope="row"><?php echo ($event['isShowcase'] == 'Y') ? '<i class="fa fa-star" title="Showcased"></i>' : '';?></td>
                                                        
                                                        <td width="55" class="table_img">
                                                            <?php
                                                            if($event['image'] && file_exists(MEDIA_FILES_ROOT.'/'.$this->_request['pageType'].'/normal/'.$event['image']))
                                                                echo '<img src="'.MEDIA_FILES_SRC.'/'.$this->_request['pageType'].'/normal/'.$event['image'].'" alt="'.$event['image'].'" height="32" />';
                                                            elseif($data['settings']['isGallery'] && $event['image'] && file_exists(MEDIA_FILES_ROOT.'/'.$this->_request['pageType'].'/normal/'.$event['image']))
                                                                echo '<img src="'.MEDIA_FILES_SRC.'/'.$this->_request['pageType'].'/normal/'.$event['image'].'" alt="'.$event['image'].'" height="32" />';
                                                            else
                                                                echo '<img src="'.ADMIN_TMPL_PATH.'/images/noicon.png" alt="'.$event['title'].'" height="32">';
                                                            ?>
                                                        </td>

                                                        <td>
                                                            <a href="index.php?pageType=<?php echo $this->_request['pageType'];?>&dtls=<?php echo $this->_request['dtls'];?>&dtaction=add&editid=<?php echo $event['id'];?>&moduleId=<?php echo $this->_request['moduleId'];?>">
