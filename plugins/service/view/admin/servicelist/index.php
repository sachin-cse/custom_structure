            ...
            <div class="col-sm-3">
                <div class="card">
                    <div class="card-body text-center">
                        <a href="<?php echo SITE_ADMIN_PATH.'/index.php?pageType='.$this->_request['pageType'].'&dtls='.$this->_request['dtls'].'&dtaction=add&moduleId='.$this->_request['moduleId'];?>" class="btn btn-info">Add Service</a>
                    </div>
                </div>
            </div>
        </div>
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <tbody class="swap">
                                                ...
                                                foreach($data['services'] as $service) {
                                                    ...
                                                    ?>
                                                    <tr id="<?php echo 'recordsArray_'.$service['id'];?>">
                                                        <td width="40">
                                                            <input type="checkbox" name="selectMulti[]" value="<?php echo $service['id'];?>" class="case" />
                                                        </td>
                                                        
                                                        <td width="40" scope="row"><?php echo $slNo;?></td>
                                                        
                                                        <td width="30" scope="row"><?php echo ($service['isShowcase'] == 'Y') ? '<i class="fa fa-star" title="Showcased"></i>' : '';?></td>
                                                        
                                                        <td width="55" class="table_img">
                                                            <?php
                                                            if($data['settings']['isIcon'] && $service['serviceIcon'] && file_exists(MEDIA_FILES_ROOT.'/'.$this->_request['pageType'].'/thumb/'.$service['serviceIcon']))
                                                                echo '<img src="'.MEDIA_FILES_SRC.'/'.$this->_request['pageType'].'/thumb/'.$service['serviceIcon'].'" alt="'.$service['serviceName'].'" height="32" />';
                                                            elseif($data['settings']['isGallery'] && $service['serviceImage'] && file_exists(MEDIA_FILES_ROOT.'/'.$this->_request['pageType'].'/thumb/'.$service['serviceImage']))
                                                                echo '<img src="'.MEDIA_FILES_SRC.'/'.$this->_request['pageType'].'/thumb/'.$service['serviceImage'].'" alt="'.$service['serviceName'].'" height="32" />';
                                                            else
                                                                echo '<img src="'.ADMIN_TMPL_PATH.'/images/noicon.png" alt="'.$service['serviceName'].'" height="32">';
                                                            ?>
                                                        </td>

                                                        <td>
                                                            <a href="index.php?pageType=<?php echo $this->_request['pageType'];?>&dtls=<?php echo $this->_request['dtls'];?>&dtaction=add&editid=<?php echo $service['id'];?>&moduleId=<?php echo $this->_request['moduleId'];?>">
                                                        ...
                                                        <td>
                                                            <?php 
                                                            //if(!$this->_request['parentId']){ ?>
                                                                <a  href="<?php echo 'index.php?pageType='.$this->_request['pageType'].'&dtls='.$this->_request['dtls'].'&moduleId='.$this->_request['moduleId'].'&parentId='.$service['id'];?>">
                                                                    Sub Service (<?php echo $service['subCount'];?>)
                                                                </a>
                                                            <?php //} ?>
                                                        </td>
                                                         */?>
